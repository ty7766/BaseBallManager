...
### 세션 40 (2026-08-18) — 미해결 항목 총정리 후 8건 일괄 처리

**브랜치**: `feature/league-single-game-flow` (`develop`에서 분기)

**착수 방식**: UI를 뺀 전 프로젝트 미해결 사항 33건을 번호로 정리 → 작성자가 항목별로 처리 방침 지시 → 그대로 진행.
작성자 요청으로 Claude가 전 구간 직접 작성.

**작성자 판단으로 종결된 항목** (코드 작업 없음)
| 항목 | 결론 |
|---|---|
| `AiRosterSet`이 `Normal` 1개뿐 | 세션 33 설계 의도대로. 티어별 차이는 `_statBonus`로만 표현 → **현 구조 유지** |
| 훈련 비용 / 한도 확장 비용 / 분해 보상 | 이미 `[SerializeField]` + 기본값 완비. **기획서에 숫자가 없었을 뿐** → 종결 |
| 포스트시즌 시리즈 경기 수 | 작성자 규칙과 구현이 **완전 일치**. 기획서 표기만 "제안" → "확정" |
| 데드락 5.3% / `velo` 표준편차 | 논외 (KBO 실제 구속 분포가 원래 좁음) |
| `Minor1~Legend3` 밸런스 | 🔒 작성자 담당 유지 |
| 타이브레이커 경기 / F항목(태그업·삼중살·사구 등) | v1.0 제외 확정 |
| 대주자 교체 | **기획서에서 삭제** (재검토 없음) |

---

#### 40-1. 기획서 정정 (D24·25·26 + 다수)

- **1.3 세부 스탯 최저 50 → 30** — 실제 CSV 최저 35. 포수 주루(41.6)·불펜 지구력(38.5)은 낮은 것이 정상이고,
  전 스탯을 50 이상으로 깔면 포지션 개성이 사라져 자동 편성이 포지션을 구분할 근거를 잃는다. **데이터가 옳으므로 문서를 고침**
- **7.2** 144경기가 27(9팀×3연전)로 나누어떨어지지 않아 상대별 15~18경기로 갈리는 점 명시. **144 유지**(KBO 실제 경기 수 우선)
- **7.3** 3연전 중에도 홈/원정이 매 경기 바뀜을 명시 — 홈 어드밴티지를 모델링하지 않아 구장 고정에 게임적 의미가 없음
- **8.3.1 도루 규칙 신설** (구현 전에 규칙부터 확정)
- **4장** 골글 제작 재료·비용 확정 표 / **5장** 시작 시그니쳐 지정 방식 / **9.1** 경기당 보상 신설
- **12장** 미정 목록 갱신 + **v1.0 제외 항목 명문화**

#### 40-2. 리그 진행 단위를 1경기로 확정 (E31 + 작성자 지시)

기획서 7.4를 "**[게임 시작] 1회 = 경기 1개**"로 확정. 일괄 시뮬은 v1.0에서 제외.

- **`SimulateNextDay`가 이미 그 단위였음** — "하루 = 내 경기 1건 + AI끼리 4경기". 새로 만들 것 없이 이름만 `SimulateNextGame`으로 변경
- 삭제: `LeagueRunner.SimulateDays` / `PostSeasonRunner.SimulateNextSeries` / `SimulateAll`
  - ⚠️ **무승부 재경기 상한 가드가 삭제된 메서드에만 있었음** → `SimulateNextGame`으로 이동
- **포스트시즌 진행도 저장 신규** — 1경기 단위가 되면서 경기 사이에 앱이 종료될 수 있게 됨
  - `PostSeasonSaveData` (DTO 3개) / `PostSeasonSaveService`
  - `PostSeasonSeries` · `PostSeasonRunner`에 복원 전용 생성자·팩토리
  - `LeagueManager`: `SimulateNextPostSeasonGame` / `SavePostSeason` / `LoadPostSeason` / `HasSavedPostSeason` / `DeleteSavedPostSeason`

📝 설계 결정:
- **세이브 키를 정규시즌과 분리** — 수명이 다름. 포스트시즌은 정규시즌이 끝난 뒤에야 생기고 재도전 시 먼저 버려진다
- **대진표를 순위표에서 다시 뽑지 않고 저장된 그대로 복원** — 다시 뽑으면 같은 대진이 나오긴 하지만,
  이미 끝난 시리즈의 승자가 다음 시리즈에 채워져 있는 상태까지는 재현되지 않는다
- **승수를 경기 기록에서 다시 세지 않고 그대로 저장** — 와일드카드는 4위가 1승을 안고 시작해 경기 결과만으로 승수가 결정되지 않는다
- `HigherSeedWins`를 1에서 시작시키는 기존 방식이 작성자 규칙(4위 1승 / 5위 2연승)과 정확히 일치함을 확인
- **`StartLeague` / `DeleteSavedLeague`가 포스트시즌 세이브를 함께 삭제** — 안 지우면 다음 시즌의 진출 여부와 무관하게 이어하기가 뜬다

#### 40-3. 디버그 토글 제거 (E28) + 주석 보강 (E30)

- `_useAiRosterForPlayerTeam` 3개 파일 + 씬에서 제거
- 🔴 **씬에 `1`(켜짐)로 남아 있었음** — 실제 플레이어 라인업이 아니라 **AI 로스터로 리그가 돌고 있었다.**
  세션 36에서 코드 기본값을 `false`로 바꿨지만 씬 인스펙터 값이 덮고 있었던 것
- `SimulationContext` XML 주석 보강 — 이름과 달리 **라인업 배열 요소가 mutable**이고,
  원본을 그대로 넘기면 대타 교체 1회가 고정 로스터를 영구 오염시킨다는 점 명시

#### 40-4. 도루 구현 (기획서 8.3.1) — 신규 기능

**완성된 파일**
- `Assets/Scripts/ProbabilityModels/StealCalculator.cs` — 시도·성공 판정 (순수 C#)
- `Assets/Scripts/Simulation/StealAttempt.cs` — 시도 1건 (`readonly struct`)
- `Assets/Scripts/Simulation/SimulationStealLog.cs` — 도루 로그

**변경**
- `GameSimulator.SimulateAtBat` 진입부에 `TryResolveSteal` — **투구 전에** 판정
- `GameResult.StealLogs` 추가 / `HitterGameStats`에 `StolenBases` · `CaughtStealing`
- `BoxScoreBuilder.Build`가 도루 로그를 함께 받음 (기본값 `null`로 기존 호출 호환)

📝 설계 결정:
- **도루 로그를 타석 로그와 분리** — 도루는 타석 시작 전의 독립 이벤트이고,
  **실패로 3아웃이 되면 그 타석 자체가 없어져** 붙일 타석 로그가 존재하지 않는다
- **실패로 3아웃이면 타순을 전진시키지 않음** — 실제 야구에서 그 타자가 다음 이닝 선두 타자로 다시 나온다.
  `TryResolveSteal`이 `false`를 반환해 `AdvanceBatter` 이전에 빠져나가므로 자동으로 충족됨
- **1·2루만, 목표 베이스가 비었을 때만, 한 타석에 한 명만** — 홈 스틸·더블 스틸 없음
- 2루 주자를 먼저 보는 이유: 1·2루 동시 주자면 1루 주자는 목표(2루)가 막혀 애초에 후보가 아님

**🔬 밸런스 — 기획서 초안 상수를 실측으로 역산 보정**

초안(`0.12` / `0.70`)으로 500경기를 돌리니 **경기당 2.31개 · 성공률 75.4%** 로 과다했다.
원인은 **실제로 베이스에 나가는 주자가 카드풀 평균보다 주루가 좋기 때문**(실측 편차 +0.30).
`StatBaseline.HitterRun`(58)은 전 카드 평균이지 "출루한 주자"의 평균이 아니다.

| 상수 | 초안 | 확정 |
|---|---|---|
| `BaseAttemptChance` | 0.12 | **0.070** |
| `BaseSuccessChance` | 0.70 | **0.625** |
| `MinAttemptChance` | 0.02 | **0.01** |

**1000경기 실측 (튜닝 후)**
| 지표 | 실측 | KBO |
|---|---|---|
| 경기당 도루 | **1.42개** | 1.2~1.6 |
| 도루 성공률 | **70.3%** | ~70% |
| 타율 / 삼진 / 볼넷 / 홈런 | .270 / 17.6% / 9.0% / 1.8% | .270 / 17.5% / 9.3% / 2.0% |
| 경기당 총득점 | **8.53** (도루 전 8.24) | ~9.0 |

- 타격 지표 **회귀 없음**. 득점이 KBO에 더 가까워짐
- 선수 변별력: 주루 35 → 시도 1% · 성공 42% / 주루 84 → 시도 33% · 성공 86% (**클램프 포화 없음**)

#### 40-5. 강화 전용 카드 5장 경로 (세션 11부터 예고만 되던 항목)

**확정: 카드가 아니라 재화로 처리** (작성자 결정)

카드로 만들면 마스터 CSV에 스탯·포지션이 없는 유령 카드가 200장 한도를 잡아먹고,
라인업·분해·필터가 전부 예외 분기를 갖게 된다. `CurrencyType`에 5종이 이미 정의돼 있었다.

- `EnhanceManager.Enhance` → **`EnhanceWithIdenticalCard`** 로 개명 (호출부 0건이라 파급 없음)
- **`EnhanceWithEnhanceCards(targetInstanceId)`** 신규 — 전용 카드 5장 소모
- `GetRequiredEnhanceCard(instanceId)` — UI 표기용 (종류 + 장수)
- `GetRequiredEnhanceCardType` — **시그니쳐·골글은 등급이 아니라 종류로** 갈린다(둘 다 5성 고정이므로)

#### 40-6. 골든글러브 제작 (로드맵 10번)

**완성된 파일**: `Assets/Scripts/GoldenGlove/` — `GoldenGloveCraftType` / `GoldenGloveCraftResult` / `GoldenGloveCraftManager`

📝 설계 결정:
- **후보 풀 확인을 재화 차감보다 먼저** — 순서를 바꾸면 골글 카드가 0장일 때 재료만 먹고 카드를 못 준다
- 비용은 `CurrencyCost[]` + `SpendAll` — 3종 동시 소모, 하나라도 부족하면 무차감 (세션 36 규약 재사용)
- 후보 버퍼를 필드로 재사용 — 제작마다 `List` 생성 회피

**비용 (인스펙터 조정 가능)**
| 제작 | 골글 포인트 | 포인트 | 훈련 카드 |
|---|---|---|---|
| 일반 (전체 랜덤) | 300 | 15,000 | 20 |
| 팀 선택 | 750 | 40,000 | 50 |

근거: 골글포인트 300 ≈ 프로 리그 우승 3회 / 팀 선택 2.5배(포지션 지정 불가라 무한정 비싸게 두지 않음) /
포인트 15,000은 훈련 1→30 누적(43,500)보다 싸게 — 제작이 포인트 주 소비처가 되면 육성이 죽는다

#### 40-7. 시작 지급 시그니쳐 에디터화

- `StartingSignatureEntry` (struct) + `StartingSignatureTable` (SO)
- `[CardId(PlayerTypeFilter.All)]` — **세션 33의 팀별 폴더 + 검색 드롭다운을 그대로 재사용**
- `GameFlowManager`의 "해당 팀 시그니쳐 중 무작위" 임시 로직 제거 → 표에서 읽음
- **지정하지 않은 팀은 카드 없이 시작**(경고만) — 시그니쳐 마스터 데이터가 없어도 게임이 돌아가야 함
- 표에 적힌 cardId가 CSV에서 사라졌으면 `LogError` (데이터 불일치)

#### 40-8. 리그 보상 확장

- `LeagueTierEntry`에 4개 필드 추가 — 골글 포인트 / 시그권 / **경기당 훈련돌파 확률** / **경기당 강화 전용 확률**
- `LeagueRewardService.GrantPerGameRewards(tier)` 신규, `LeagueManager.SimulateNextGame`에서 호출
- 종료 보상에 골글 포인트·시그권 추가 (골드와 같은 순위 배수 적용)

📝 설계 결정:
- **경기당 보상은 승패와 무관하게 지급** — 훈련돌파·강화 전용 카드는 육성 필수 재료라,
  못 이기는 구간에서 보급이 끊기면 영영 따라잡을 수 없게 된다
- 강화 전용 카드 등급 가중치 **3성 60% / 4성 30% / 5성 10%**
- **시그니쳐 전용은 경기 보상에서 제외** — 시그 카드 자체가 뽑기 한정이라 보급이 앞서면 안 됨.
  골카 전용은 기획서 9.1대로 리그 우승 보상 전용

---

### ✅ 세션 40 검증 — dotnet 하니스 재구축 + 101건 통과

세션 38의 하니스가 스크래치패드 전용이라 소멸해 있어 **처음부터 다시 세움**
(`UnityEngine` 최소 셰임 + `JsonUtility` 재현 + 씨앗 CSV → 로스터 구성).

런타임 **전량 컴파일 — 오류 0 · 경고 0**

| 스위트 | 건수 | 내용 |
|---|---|---|
| 실경로 관통 | 24 | 신규 시작 → 뽑기 → 자동 편성 → 리그 입장 → 1경기 → 9경기 완주 → 보상·해금 → 세이브 왕복 |
| 포스트시즌 | 24 | 대진 구성 / 1경기 진행 / 중간 저장 / 복원 일치 / 이어서 종료 / 로테이션 보존 |
| 재화·제작·보상 | 45 | 전용 카드 등급 매칭·무차감 실패·만렙 거부 / 골글 제작 3종 차감 / 시그 표 / 경기당 보상 |
| 도루 | 8 | 규칙 5종 + 밸런스 2종 (1000경기) |

**골글 제작 성공 경로는 카드를 임시 주입해 검증** — CSV에 골글 카드가 0장이라 실패 경로만으로는
"카드를 넣는 순간 터지는" 위험이 남는다. 마스터 딕셔너리에 골글 9장 + 시그 3장을 주입해
제작 성공 · 팀 필터 · 3종 재화 정확 차감 · 골카/시그 전용 카드 매칭까지 관통 확인했다.

**🐛 테스트 기대값 오류 1건** — 와일드카드를 "무조건 1경기"로 단언했으나,
5위가 1차전을 잡으면 1-1이 되어 2차전이 열리는 것이 정상. **프로덕션 코드는 수정하지 않았다**
(세션 37과 같은 유형).
...
| # | 항목 | 상태 |
|---|---|---|
| 1 | `LeagueTierTable.asset`에 경기당 보상 확률 입력 | ✅ 완료 |
| 2 | 같은 에셋에 골글 포인트 / 시그권 티어별 값 입력 | ✅ 완료 |
| 3 | `StartingSignatureTable.asset` 생성 + `GameFlowManager`에 연결 | ✅ 에셋·배선 완료 (**팀별 지정은 시그 카드 입력 후**) |
| 4 | `GoldenGloveCraftManager`를 씬에 배치 | ✅ 완료 (매니저 14종) |
| 5 | `Minor1~Legend3` 밸런스 | 🔒 작성자 담당 (`_statBonus`는 여전히 9티어 전부 10) |

---

### 세션 40 (계속) — 에디터 수치 입력 완료 (작성자)

**씬 무결성 검증** — 앵커 55개(중복 0) / SceneRoots 16개 전부 해소 / 매니저 **14종** / dangling 참조 0

**골든글러브 제작 비용 (작성자 확정)**

| 제작 | 골글 포인트 | 포인트 | 훈련 카드 |
|---|---|---|---|
| 일반 | 50,000 | 1,000,000 | 100 |
| 팀 선택 | 100,000 | 5,000,000 | 300 |

Claude 제안값(300 / 15,000 / 20)보다 크게 높다. **골든글러브는 엔드 콘텐츠이므로 쉽게 만들 수 있으면 안 된다**는 작성자 판단.
환산: 골글 포인트 50,000 ≈ 레전드3 우승 10회(1,440경기) / 포인트 1,000,000 ≈ 5성 1,250장 분해 ≈ 카드 23장 풀훈련.
→ 기획서 4장의 비용 표를 실제 값으로 정정함.

**리그 보상 (작성자 입력, 21행)**
- 골드 3,000(베이직1) ~ 250,000(레전드3)
- 골글 포인트는 **프로1부터** 지급 (500 ~ 5,000). 골글 제작을 중후반으로 게이팅
- 시그니쳐 뽑기권 0 ~ 15
- 경기당 보상 확률 50% ~ 100%

**📌 `Basic1`의 경기당 보상 0%는 의도된 값** (작성자 확인).
9경기짜리 첫 리그는 맛보기라 육성 재료를 주지 않는다. 세션 39의 `Amateur3` `_statBonus` 누락과는 **다른 성격**이므로 고치지 말 것.

**⏭️ `StartingSignatureTable._entries`는 비어 있음** — 에셋·배선은 끝났고, 지정할 시그니쳐 카드가 CSV에 아직 없어 보류.
현재는 모든 팀이 시작 카드 없이 출발한다(`LogWarning` 후 진행).

**밸런스 검산 (입력값 기준)**
| 항목 | 값 |
|---|---|
| 강화 전용 카드 공급 (마이너1) | 시즌당 약 125장 → 5성 1장 풀강(50장)에 **시즌 4회** |
| 훈련돌파 카드 공급 (마이너1) | 시즌당 약 125장 → 노말 5성 **12.5장** 돌파 가능 |
| 골글 제작 (레전드3 우승 기준) | 골글 포인트 10회 / **포인트가 실질 병목** |

**데이터 입력 (작성자, 천천히)**
- 시그니쳐 / 골든글러브 카드 마스터 CSV — 현재 **노말 325장뿐**
- cardId는 **append-only** (세션 33 규칙): 타자 `160~` / 투수 `50167~`

**남은 로드맵**: 9번 경기 UI(디자인 대기) / 11번 튜토리얼(최종)
...
### 세션 41 (2026-08-25) — `GameSession` 구현 (로드맵 9번 선행)

**착수 판단**: 세션 39에서 "UI 디자인 대기"로 보류했던 항목이지만, `GameSession`은 순수 C#이고
화면에 대해 아무것도 모른다. UI가 시뮬에게 요구하는 것은 ① 현재 상태 ② 한 칸 전진 ③ 방금 생긴 로그
세 가지로 고정이므로 디자인이 어떻게 나오든 이 셋은 바뀌지 않는다. **디자인보다 먼저 만드는 것이 맞다.**

**완성된 파일**
- `Assets/Scripts/Simulation/GameSession.cs` — 경기 1건의 진행 상태를 들고 타석 단위로 전진 (순수 C#)

**변경된 파일**
- `Assets/Scripts/Simulation/GameSimulator.cs`
  - `SimulateAtBat` `private` → **`internal`** (프로젝트 밖에 내놓는 API가 아니라 `GameSession`에게만 빌려주는 통로)
  - `SimulateGame`이 `GameSession`을 경유하도록 재작성 — 진행 규칙을 한 벌로 유지

**공개 멤버 (GameSession)**

| 멤버 | 역할 |
|---|---|
| `IsGameOver` / `State` | UI가 읽는 진행 상태 |
| `BatterLogs` / `StealLogs` | 경기 전체 누적 로그 (`IReadOnlyList`) |
| `LastStepStealLogs` / `LastStepBatterLog` | **이번 스텝 몫** — UI가 화면에 흘릴 대상 |
| `StepAtBat()` | 타석 1회 전진. 이미 끝난 경기면 `false` |
| `BuildResult()` | 종료 후 `GameResult` 조립, 진행 중이면 `null` |

📝 주요 설계 결정:
- **`StepAtBat()`의 반환값을 로그가 아니라 `bool`로** — 세션 39 설계("방금 끝난 타석 로그 반환")는 **도루 도입 이전** 것이다.
  세션 40 이후 한 스텝이 남기는 로그는 `타석 1 + 도루 0` / `타석 1 + 도루 1` / **`타석 0 + 도루 1`(도루 실패 3아웃)** 로 갈려
  "로그 1개 반환"으로 표현할 수 없다. `bool` + `LastStep*` 프로퍼티 조합으로 분리
- **`LastStep*`을 호출 전 개수 스냅샷으로 산출** — `_batterLogs[^1]`을 읽으면 한 타석이 로그를 2개 남기도록
  바뀌었을 때 조용히 틀린다 (세션 39 경고 반영). 호출 전 `Count`를 잡고 그 인덱스부터 꺼낸다
- **타석 로그가 2개 이상 늘면 `Debug.LogError`** — `LastStepBatterLog`가 단수라 뒤쪽을 버리게 되는 상황을
  목록 반환으로 복잡도를 올리는 대신 즉시 드러내는 쪽 선택 (3-2 "데이터 불일치·버그 의심" 규약)
- **`SimulateGame`을 `GameSession` 경유로 재작성** — 안 하면 일괄 시뮬과 실시간 관전이 서로 다른 루프를 갖게 되어
  언젠가 규칙이 갈라진다. `while (session.StepAtBat()) { }` 한 줄로 축약됨
- **생성자 인자 순서 `(context, simulator)`** — `GameState`도 `context`를 받으므로 "경기 데이터 먼저, 도구 나중"
- `_batterLogs` 초기 용량 80 — 9이닝 경기 타석이 70~80개. 기본 용량 4에서 두 배씩 늘면 경기마다 재할당 5~6회

**🔬 검증 (dotnet + UnityEngine 셰임, 스크래치패드)**

`Simulation/` + `ProbabilityModels/` 전량 컴파일 — **오류 0 · 경고 0**

| 스위트 | 결과 |
|---|---|
| 인자 방어 (`null` context/simulator → `ArgumentNullException`) | ✅ 2건 |
| 진행 전 상태 (로그 비어있음 / `LastStepBatterLog == null` / `BuildResult() == null`) | ✅ 4건 |
| 타석 단위 전진 (로그 단조 증가 / `LastStep*` 증가분 일치 / 종료 후 `false`) | ✅ 7건 |
| `BuildResult` (점수·로그 개수가 `State`와 일치) | ✅ 4건 |
| **구 루프 vs `GameSession` 경유 동등성 — 300경기 전부 완전 일치** | ✅ **동작 회귀 없음** |

- `스텝 수 == 타석 로그 수 + 로그 없는 스텝 수` 항등식 성립 확인 (도루 실패 3아웃 경로 포함)
- ⚠️ 합성 로스터(랜덤 스탯 45~90, 불펜 지구력 40) 기준 경기당 득점 10.99가 나왔으나
  **이는 테스트 픽스처 탓**이다. 실제 카드풀 기준 8.53(세션 40 실측)이고, 동등성 검증이 300경기 완전 일치를
  보였으므로 밸런스 회귀가 아니다. 프로덕션 코드 미수정

**⏭️ 다음** — UI 디자인 확정 후 로드맵 9번 착수. `GameSession`은 준비 완료 상태

---

### 세션 41 (계속) — 시작 지급 150장으로 상향 (데드락 항목 종결, 작성자 결정)

**배경 — 데드락이 실제로 탈출구 0개임을 확인**

| 뽑기권 획득 경로 | 상태 |
|---|---|
| 리그 보상 | ❌ 라인업 20칸을 채워야 입장 가능 |
| 카드 분해 | ❌ `DismantleResult`는 `Point` / `TrainCard` / `GoldenGlovePoint`만 반환 |
| 재화 교환소 | ❌ 기획서 12장 v1.0 제외 확정 |

라인업을 못 채운 신규 유저는 뽑기권을 다시 얻을 방법이 없어 세이브 삭제 외에 진행 수단이 없었다.

**결정: 시작 지급 100장 → 150장** (작성자 결정. 구제 지급·분해 보상 추가 등 코드 변경 안은 채택하지 않음)

**변경**
- `Assets/Scripts/Player/GameFlowManager.cs:20` — `_tutorialTicketReward = 100` → **150**
- `Assets/Scenes/SampleScene.unity:151` — `_tutorialTicketReward: 100` → **150**
- `베이스볼_매니저_기획서.md` 5장 — **"일반 뽑기권 50개 지급"이 세션 39의 100장 변경 때 갱신 누락돼 있었음.**
  150으로 정정하면서 시작 지급량이 신규 유저 진입 관문이라는 근거와 실측 완성률 표를 함께 명시

📝 참고 (근거 기록용, 재논의 대상 아님):
- 세션 39 실측 곡선: 50장 37.1% / 100장 94.7% / 120장 97.1% (각 1000회)
- 같은 표본에서 **완성에 필요한 카드 수 최대치가 200장**이었으므로, 150장도 완성률 100%는 아니다.
  다만 잔여 비율은 100장 기준 5.3%보다 확연히 낮고, 작성자 판단으로 **이 항목은 여기서 종결**한다
- 인벤 점검: 150장 + 시작 시그니쳐 1장 = 151장 / 한도 200장. 초과 없음(여유 49칸)

**⏭️ 남은 미결** — 무승부 상대전적 표시(`TeamRecord._drawsAgainst`)는 **UI 디자인 확정 후 판단**

---

### 세션 41 (계속) — 카드 조합 시스템 신규 구현 (기획서 2.3 신설)

**요청**: 카드 3장을 조합해 랜덤 1장. 상위 등급도 확률로 나옴. 노말/시그/골글은 종류끼리만.

**완성된 파일**
- `Assets/Scripts/Combine/CombineResult.cs` — 조합 1회 결과 (InstanceId / CardId / CardType / BaseGrade / Grade / IsUpgraded)
- `Assets/Scripts/Combine/CombineManager.cs` — 싱글톤 MonoBehaviour

**완성된 메서드 (CombineManager)**
- `GetMaterialCount()` — UI 슬롯 구성용
- `CanCombine(materialInstanceIds)` — 개수 · 중복 · 존재 · 잠금 · 라인업 · 종류 일치 검증
- `Combine(materialInstanceIds)` — 기준 등급 추첨 → 승급 → 후보 풀 → 재료 소멸 → 카드 지급
- `DecideResultGrade(cardType, baseGrade)` — 노말만 승급, 시그·골글은 그대로
- `CollectCandidates(cardType, grade)` — 타자·투수 통합 후보 풀 구성

**확률 모델 — "무작위 재료 1장의 등급"을 기준으로 승급**

작성자 요청: *"4성, 5성, 5성을 넣으면 3성은 안 나오고 5성이 나올 확률이 높다"*

재료 3장 중 무작위 1장을 뽑아 그 등급을 기준으로 삼고, 거기서 승급을 굴린다.
별도 가중치 표 없이 ① 상위 재료가 많을수록 상위 확률 상승 ② 재료에 없는 하위 등급은 불출 이 동시에 성립한다.
3성이 4성으로 오르면 한 번 더 굴려 5성까지 갈 수 있어(연쇄) 3성 3장으로도 5성이 나온다.

| 필드 | 기본값 |
|---|---|
| `_upgradeChanceStar3` (3→4) | 25% |
| `_upgradeChanceStar4` (4→5) | 10% |
...
작성자가 든 예시(4/5/5)가 정확히 재현됨 — 3성 불출, 5성 70.2%.

📝 주요 설계 결정:
- **등급 혼합 허용** — 처음엔 "3장 등급 동일 필수"로 제안했으나 작성자 규칙에 맞춰 철회
- **시그·골글은 5성 고정(1.2)이라 승급 개념이 없음** → 같은 종류 안에서 리롤. **중복 허용**(작성자 확인)
- **중복 재료 검사를 존재·잠금 검사보다 먼저** — 순서가 반대면 카드 1장이 모든 검사를 통과한 뒤
  `RemoveCard`가 3번 호출돼 1장으로 3장짜리 조합이 성립한다 (**카드 복제 버그**)
- **중복 검사는 `HashSet` 대신 이중 for문** — 재료가 3개뿐이라 비교 3회. `HashSet`은 힙 할당이 발생해 손해
- **후보 풀 확인을 재료 소멸보다 먼저** — 순서를 바꾸면 해당 종류·등급 마스터가 없을 때 재료만 먹고 카드를 못 준다
  (세션 40 골글 제작에서 얻은 교훈 재적용)
- **인벤 한도 체크 불필요** — 3장이 빠진 뒤 1장이 들어오므로 항상 여유가 생김
- **추가 재화 비용 없음** — 카드 3장 자체가 비용

⚠️ **밸런스 경고 (작성자 판단 필요)**: 포인트의 유일한 획득 경로가 **분해**다.
승급 확률이 후하면 잉여 3성 카드가 전부 조합으로 흘러 분해가 죽고 포인트 경제가 멈춘다.
기본값 25%/10%은 그 점을 감안한 보수적 시작값이며, 인게임 확인 후 조정 대상(기획서 12장 TBD 추가).

⚠️ **골글 조합은 사실상 죽은 기능**: 골글은 제작으로만 얻고 1장 비용이 골글포인트 50,000 + 포인트 1,000,000
+ 훈련카드 100이다. 3장을 태워 1장을 받으면 순손실 2장. 규칙 일관성 때문에 막지 않았을 뿐 권장 경로가 아니다.

**🔬 검증 (dotnet + UnityEngine 셰임, 실제 CSV 로드)**

`Combine` + `Cards` + `Inventory` + `Line Up` + `Currency` + `CSVParse` 컴파일 — **오류 0**

| 스위트 | 결과 |
|---|---|
| 규칙 검증 (정상 조합 / 재료 수 / null / 중복 / 종류 혼합 / 등급 혼합 / 잠금 / 시그 / 골글 / 중복 결과) | ✅ 18건 |
| 결과 등급 분포 (5가지 재료 구성 × 20,000회) | ✅ 6건 |
| **합계** | **24 / 24 통과** |

- **카드 복제 방지 실증** — 같은 카드 3번 지정 시 거부되고 인벤 개수가 불변임을 확인
- 인벤 순감 2장(3 → 1) · 재료 3장 전부 소멸 · 결과 카드 실존 확인
- CSV에 시그·골글이 0장이라 **마스터 딕셔너리에 임시 주입**해 성공 경로까지 관통 (세션 40과 같은 방식)
- 정상 경로에서 `LogError` 0건

**기획서 갱신**
- **2.3 카드 조합 신설** — 재료 규칙 · 결과 등급 결정 · 실측 분포 표 · 밸런스 주의
- 12장 확정 목록에 카드 조합 추가 / TBD에 승급 확률 튜닝 추가

**⏭️ Unity에서 할 것**: `CombineManager`를 씬에 배치 (매니저 14종 → 15종)

---

## 📋 Unity 에디터 작업 대기 목록 (세션 41 기준)

> 세션 41은 **코드·문서만** 작업했다. 아래는 에디터를 열었을 때 처리할 것들이다.

### 🔴 먼저 확인 — 씬 파일을 텍스트로 직접 수정함

세션 41에서 `SampleScene.unity`의 `_tutorialTicketReward`를 **100 → 150**으로 파일에서 직접 고쳤다.
**수정 시점에 Unity 에디터가 열려 있었다면** 에디터의 메모리 상태가 파일을 덮어써 되돌아갈 수 있다.
에디터를 열면 `GameFlowManager` 인스펙터에서 **150인지 먼저 확인**할 것. (코드 기본값도 150이라 새로 붙이면 150이 맞다)

### ⏭️ 할 일

| # | 항목 | 비고 |
|---|---|---|
| 1 | `CombineManager`를 씬에 배치 | 세션 41 신규. 매니저 **14종 → 15종**. 승급 확률은 기본값(25% / 10%)으로 시작 |
| 2 | `.meta` 생성 확인 | `GameSession.cs` · `CombineManager.cs` · `CombineResult.cs` 3개. 에디터 포커스 시 자동 생성 |
| 3 | 시그니쳐 · 골든글러브 마스터 CSV 반영 | **골글은 시트 작성 완료 / 시그는 작성 중** (2026-08-25 기준). cardId **append-only** — 타자 `160~` / 투수 `50167~` |
| 4 | `StartingSignatureTable._entries` 채우기 | **3번이 끝나야 가능.** 현재 비어 있어 모든 팀이 시작 카드 없이 출발(`LogWarning` 후 진행) |
| 5 | `Minor1~Legend3` `_statBonus` 밸런스 | 🔒 **작성자 담당.** 현재 9티어 전부 `10`으로 동일 — 티어가 올라가도 AI가 강해지지 않는다 |

### 💤 코드 쪽 대기 (에디터 아님)

| 항목 | 상태 |
|---|---|
| 로드맵 9번 경기 UI | ⏸️ **UI 디자인 대기.** `GameSession`은 준비 완료 |
| 무승부 상대전적 표시 (`TeamRecord._drawsAgainst`) | ⏸️ UI 디자인에 "1무" 자리가 있는지 확정 후 판단 |
| 로드맵 11번 튜토리얼 | ⏭️ 최종 |
