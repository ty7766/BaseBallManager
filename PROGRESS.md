...
| # | 항목 | 비고 |
|---|---|---|
| 1 | `LeagueTierTable.asset`에 **경기당 보상 확률** 입력 | 🔴 전부 0이라 훈련돌파·강화 전용 카드가 **영영 안 나온다** |
| 2 | 같은 에셋에 **골글 포인트 / 시그권** 티어별 값 입력 | 🔴 0이면 골글 제작이 불가능 |
| 3 | `StartingSignatureTable.asset` 생성 + `GameFlowManager`에 연결 | 시그 카드 입력 후 |
| 4 | `GoldenGloveCraftManager`를 씬에 배치 | 세션 39의 매니저 배선에 추가 필요 |
| 5 | `Minor1~Legend3` 밸런스 | 🔒 작성자 담당 |
