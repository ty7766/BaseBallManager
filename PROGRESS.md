...
| # | 항목 | 현재 상태 |
|---|---|---|
| 1 | `GachaManager.Roll10` null 반환 3곳 | 🐛 위 참조 |
| 2 | 시작 뽑기권 100장 상향 | ✅ 완료 (씬 반영·실측 94.7%) |
| 2-1 | `GameFlowManager.cs:20` 코드 기본값도 100으로 | 씬만 100, 코드는 50 |
| 2-2 | 100장으로도 못 채우는 5.3% 탈출구 | 미결 (데드락) |
| 3 | CSV 시그니쳐 카드 0장 | 데이터 미입력 |
| 4 | `LeagueTierTable.asset` → `_goldReward` | **에셋에 직렬화조차 안 됨** = 우승 보상 0골드 |
| 5 | `LeagueTierTable.asset` → `_statBonus` | 21개 티어 **전부 0** = 기획서 7.8 "티어↑ 능력치↑" 무효 |

4·5번은 Unity를 한 번 열어 인스펙터에서 값을 넣어야 에셋에 기록된다.
