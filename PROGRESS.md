...
기능 테스트 **77개 전부 통과** (세션 36의 69개 + 박스스코어 8개)
...
| 타점 ≤ 팀 득점 / 타수 ≤ 타석 | 기록 규칙 준수 |

경기당 평균: 득점 7.21 · 안타 17.83 · 실책 0.61 · 타석 78.3 / 연장 15건(200경기 중), 최장 11회

---

## 🔴 세션 37에서 발견한 규칙 오류 — 홈팀이 이겨도 마지막 회 말을 친다

`InningScore.HomePlayed`를 만들면서 드러났다. **200경기 전부 마지막 이닝 말 공격을 치렀다(생략 0건).**

`GameState.AddOut()`은 초 이닝이 끝나면 **점수와 무관하게** 말 이닝으로 넘긴다.
경기 종료 판정은 말 이닝이 끝난 뒤에만 하므로, 홈팀이 9회초까지 리드 중이어도 9회말을 진행한다.
실제 야구에서는 이 경우 경기가 그대로 끝난다.
...
- **영향**: 승패는 안 바뀌지만 **득실차가 부풀려진다.** 득실차는 순위 타이브레이커 2순위(기획서 7.7)이므로 홈경기가 많은 팀이 유리해짐. 홈 타자 개인 기록도 함께 부풀려짐
- **수정 범위**: `GameState.AddOut()`의 `if (IsTopInning)` 블록에서 "9회 이상 + 홈팀 리드"면 `IsGameOver = true` 처리 (3줄)
- ⚠️ **아직 고치지 않음** — 세션 35의 밸런스 실측치(경기당 총득점 8.24 등)가 미세하게 바뀌므로 작성자 판단 후 반영

---

## ⏭️ 다음 세션 착수 지점

**로드맵 9번 남은 것 — UI 뼈대 설계부터**
- 화면 구성(어떤 화면이 몇 개, 무엇을 띄우는지)을 먼저 도출한 뒤 스크립트 작성
- 데이터는 준비 완료: 실시간 로그(`SimulationBatterLog`) / 박스스코어(`BoxScore`)
- 로그 문자열 포맷은 UI 설계 시 함께 결정

**Unity에서 확인할 것 (코드 아님)**
1. 컴파일 통과 확인 후 새 매니저 4종을 씬에 배치 — `CurrencyManager` / `DismantleManager` / `GameFlowManager` (+ 기존 매니저 전부)
2. `LeagueTierTable.asset`에 **티어별 `_goldReward`** 입력 (현재 전부 0 → 보상 골드 0)
3. `Assets/Data/` 및 **신규 `.cs`의 `.meta`** 커밋 포함 (Unity를 한 번 열어야 .meta 생성됨)

**남은 로드맵**: 9번 경기 UI(로그·박스스코어) / 10번 골든글러브 제작 / 11번 튜토리얼
